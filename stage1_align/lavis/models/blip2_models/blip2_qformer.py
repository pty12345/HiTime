...
class Blip2Qformer(Blip2Base):
    ...
    def forward(self, samples):
        ...
        # loss_itc=torch.zeros((1, )).to(device)
    ...
    def generate(
        ...
        """
        ts = samples["ts"]
        ts_embeds = self.ts_ln(self.ts_encoder(ts))
    ...
    @torch.no_grad()
    def get_similarity_score(self, samples):
        ts = samples["ts"]
        label = samples["label"]
        text = samples["text_input"]
        ...
        sim_q2t = torch.matmul(
            ts_feats.unsqueeze(1), text_feats.unsqueeze(-1)
        ).squeeze()
        ...
        sim_t2q = torch.matmul(
            text_feats.unsqueeze(1).unsqueeze(1), ts_feats.permute(0, 2, 1)
        ).squeeze()
        ...
        sim_i2t = sim_i2t.detach().cpu().numpy()
        sim_t2i = sim_t2i.detach().cpu().numpy()
        
        return (np.diagonal(sim_i2t) + np.diagonal(sim_t2i)) / 2
    ...
    def from_config(cls, cfg):
        ...
        freeze_ts = cfg.get("freeze_ts", True)
