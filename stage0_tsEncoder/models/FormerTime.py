...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.data_shape = (args.seq_len, args.enc_in)
        self.d_encoder = [args.d_model, args.d_model, args.d_model]
        self.slice_per_stage = [16, 2, 2]
        self.slice_sizes = [(i, j) for i, j in zip(self.slice_per_stage, [self.data_shape[1]] + self.d_encoder)]
        self.tr = [2, 1, 1]
        self.stride = [8, 2, 2]
        self.layer_per_stage = [2, 2, 2]
        ...
        self.encs = nn.ModuleList([
            Encoder(slice_size=self.slice_sizes[i], data_shape=self.data_shapes[i], d_encoder=self.d_encoder[i],
                       ...
                       position_location=self.position, position_type='cond')
