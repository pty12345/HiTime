...
from m2t.models.src.models import Trm_Classifier_patch_v0
...
class WrappedLlamav2Config(LlamaConfig):
    ...
    mm_hidden_size: int = 512  # size of the jukebox embeddings with temporal averaging
...
class WrappedLlamav2Model(LlamaModel):
    ...
    def initialize_adapter_modules(
        ...
        if pretrain_mm_mlp_adapter is not None:
            ...
            print("[INFO] load pretrain mm_projector weight.")

        return dict(
            audio_config=AudioEncoderConfig(),
        )

    def initialize_ts_encoder_modules(
        self,
        pretrain_ts_encoder=None,
        freeze_ts_encoder=False,
    ):
        
        if not hasattr(self, "ts_encoder"):
            self.ts_encoder = Trm_Classifier_patch_v0(self.config.ts_encoder_config)
        if pretrain_ts_encoder is not None:
            ts_encoder_weights = torch.load(pretrain_ts_encoder, map_location="cpu")
            self.ts_encoder.load_state_dict(ts_encoder_weights)
            print("[INFO] load pretrain ts_encoder weight.")
        if freeze_ts_encoder:
            for p in self.ts_encoder.parameters():
                p.requires_grad = False
            print("[INFO] freeze ts_encoder weight.")
    ...
    def forward(
        ...
        # ts_embeds = self.ts_encoder(audio_encodings)
        # ts_embeds has shape [batch_size, model_dim].
        if audio_encodings is not None and self.config.use_mm_proj:
            if isinstance(audio_encodings, list):
                ts_features = [
                    self.mm_projector(self.ts_encoder(time_serie)) for time_serie in audio_encodings
                ]
            else:
                ts_embeds = self.ts_encoder(audio_encodings)
                # print(f"[DEBUG] ts_embeds.shape: {ts_embeds.shape}")
                ts_features = self.mm_projector(ts_embeds)
            ...
            for cur_input_ids, cur_input_embeds in zip(input_ids, inputs_embeds):
                if self.audio_encoder_config.use_audio_start_end:
                    ...
                    cur_audio_features = ts_features[cur_audio_idx]
                    num_frames = cur_audio_features.shape[0]
                    # print(f"[DEBUG] cur_input_ids: {cur_input_ids}")

                    # print(f"[DEBUG] audio_start_token: {self.audio_encoder_config.audio_start_token}")
                    # print(f"[DEBUG] audio_end_token: {self.audio_encoder_config.audio_end_token}")
                    # print(f"[DEBUG] audio_start_token num: {(cur_input_ids == self.audio_encoder_config.audio_start_token).sum()}")
                    # print(f"[DEBUG] audio_end_token num: {(cur_input_ids == self.audio_encoder_config.audio_end_token).sum()}")
                    ...
                    if len(audio_start_tokens):
                        ...
                        for audio_start_token_pos in audio_start_tokens:
                            cur_audio_features = ts_features[cur_audio_idx].to(
